...
import { formatTime, calculateStats } from '../utils/timerHelpers';
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 mb-4">
        <div className="flex gap-1 bg-white/5 rounded-xl p-1">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm transition-all ${
              filter === 'preset' ? 'bg-purple-500 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Presets
          </button>
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm transition-all ${
              filter === 'custom' ? 'bg-purple-500 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
            Custom
          </button>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="px-4 py-1.5 bg-white/5 border border-white/10 rounded-lg text-white placeholder-gray-500 text-sm focus:outline-none focus:border-purple-500"
          />
          <button
            onClick={() => {
              if (window.confirm('Clear all history?')) clearHistory();
            }}
            className="px-4 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-sm transition-all"
          >
            Clear
          </button>
        </div>
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-xs px-2 py-0.5 rounded-full ${
                    timer.type === 'preset' ? 'bg-purple-500/20 text-purple-300' : 'bg-pink-500/20 text-pink-300'
                  }`}>
                    {timer.type}
                  </span>
                </div>
